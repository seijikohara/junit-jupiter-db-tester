...
### [SimpleScenarioTest.java](src/test/java/example/feature/SimpleScenarioTest.java)
Demonstrates scenario-based testing with CSV filtering using the `[Scenario]` column marker. Shows how to share a single CSV file across multiple test methods, with each test loading only rows matching its method name. Reduces CSV file duplication and demonstrates automatic scenario filtering.

### [BasicConventionTest.java](src/test/java/example/feature/BasicConventionTest.java)
Demonstrates the simplest convention-based database testing. Shows automatic CSV file resolution based on test class and method names, default `@Preparation` and `@Expectation` usage, and single table operations with minimal configuration. Perfect starting point for understanding the framework.
...
Demonstrates NULL value and empty string handling in CSV files. Shows using `[null]` notation to represent SQL NULL values, distinguishing between NULL and empty string (""), handling NOT NULL constraints, and NULL values in numeric columns.
...
Demonstrates custom expectation paths for flexible test data organization. Shows using `@DataSet(resourceLocation = ...)` to specify custom paths for expectation data, organizing multiple expectation scenarios in subdirectories, multi-stage testing with different expected states, and complex validation with database state changes.

### [ProgrammaticAssertionApiTest.java](src/test/java/example/feature/ProgrammaticAssertionApiTest.java)
Demonstrates the programmatic `DatabaseBridge` API for scenarios requiring more flexibility than annotations. Shows direct usage of assertion methods without relying solely on `@Expectation` annotation, loading datasets programmatically, and combining annotation-based and programmatic approaches.
...
Demonstrates table ordering strategies for foreign key constraints. Shows automatic alphabetical table ordering (default), manual ordering via `table-ordering.txt` file, programmatic ordering in custom directories, and handling complex table dependencies including many-to-many relationships with junction tables.

### [MultipleDataSourceTest.java](src/test/java/example/feature/MultipleDataSourceTest.java)
Demonstrates using multiple named data sources in a single test. Shows registering multiple named data sources, using `dataSourceName` in `@DataSet` annotations, and working with different databases simultaneously. Useful for multi-tenant applications, microservices with separate databases, or testing data synchronization.

### [InheritedAnnotationTest.java](src/test/java/example/feature/InheritedAnnotationTest.java)
Demonstrates annotation inheritance from a base test class (`InheritanceTestBase`). Shows inheriting `@ExtendWith(DatabaseTestExtension.class)`, class-level `@Preparation` annotation, database setup and utility methods, and how to override inherited annotations at the method level.

### [NestedConventionTest.java](src/test/java/example/feature/NestedConventionTest.java)
Demonstrates `@Nested` test classes with convention-based data loading. Shows using nested classes for logical test grouping, convention-based CSV resolution for nested classes, different scenarios within nested test groups, and shared database setup across nested classes.

### [ErrorHandlingTest.java](src/test/java/example/feature/ErrorHandlingTest.java)
Demonstrates error handling and validation scenarios. Shows proper exception handling for missing directories, invalid resource locations, and other error conditions. Validates that the framework provides clear error messages for common configuration mistakes.
...
Demonstrates Apache Derby integration for embedded database testing. Shows Derby-specific configuration, in-memory database setup, and integration test patterns suitable for lightweight testing scenarios without external dependencies.

### [database/hsqldb/HSQLDBIntegrationTest.java](src/test/java/example/database/hsqldb/HSQLDBIntegrationTest.java)
Demonstrates HSQLDB (HyperSQL) integration for embedded database testing. Covers HSQLDB-specific features, in-memory mode configuration, and fast integration testing patterns ideal for CI/CD pipelines requiring minimal setup.

### [database/mysql/MySQLIntegrationTest.java](src/test/java/example/database/mysql/MySQLIntegrationTest.java)
Demonstrates MySQL integration using Testcontainers for containerized database testing. Shows MySQL-specific data type handling, connection management, and integration test configuration suitable for CI/CD environments.
...
Demonstrates Oracle Database integration using Testcontainers. Shows Oracle-specific configuration, connection management, and integration test patterns for containerized Oracle instances. Validates Oracle compatibility as a smoke test.

### [database/mssql/MSSQLServerIntegrationTest.java](src/test/java/example/database/mssql/MSSQLServerIntegrationTest.java)
Demonstrates Microsoft SQL Server integration using Testcontainers. Shows SQL Server-specific configuration, connection management, and integration test patterns for containerized SQL Server instances. Validates SQL Server compatibility as a smoke test.
...
```bash
./gradlew :example:test --tests example.feature.SimpleScenarioTest
./gradlew :example:test --tests example.feature.BasicConventionTest
./gradlew :example:test --tests example.database.mysql.MySQLIntegrationTest
```

Run a specific test method:
```bash
./gradlew :example:test --tests example.feature.SimpleScenarioTest.shouldCreateActiveUser
```
...
```
example/feature/SimpleScenarioTest/
├── TABLE1.csv                # Preparation data
└── expected/
    └── TABLE1.csv            # Expected data

example/feature/BasicConventionTest/
├── TABLE1.csv                # Preparation data
└── expected/
    └── TABLE1.csv            # Expected data
