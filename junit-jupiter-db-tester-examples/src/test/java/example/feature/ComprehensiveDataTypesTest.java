...
public final class ComprehensiveDataTypesTest {
   ...
   *
   * <p>Validates that CSV can represent all data types including:
   *
   * <ul>
   *   <li>All integer types (TINYINT, SMALLINT, INTEGER, BIGINT)
   *   <li>All decimal types (DECIMAL, NUMERIC)
   *   <li>All floating point types (REAL, FLOAT, DOUBLE, DOUBLE PRECISION)
   *   <li>All character types (CHAR, VARCHAR, VARCHAR_IGNORECASE, LONGVARCHAR, CLOB, TEXT)
   *   <li>All date/time types (DATE, TIME, TIMESTAMP)
   *   <li>Boolean types (BOOLEAN, BIT)
   *   <li>Binary type (BLOB) with Base64 encoding
   *   <li>UUID values (stored as VARCHAR)
   *   <li>NULL values (empty column in CSV)
   * </ul>
   *
  ...
  @Preparation
  @Expectation
  void shouldHandleAllDataTypes() throws Exception {
    logger.info("Running comprehensive data types test");

    try (final Connection connection = dataSource.getConnection();
        final var statement = connection.createStatement()) {
      // Insert a new record with all supported data types
