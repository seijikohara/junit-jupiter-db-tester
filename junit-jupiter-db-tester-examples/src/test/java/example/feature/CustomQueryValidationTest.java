...
public final class CustomQueryValidationTest {
  ...
  void shouldValidateRegionalSales() throws Exception {
  ...
  void shouldValidateSalesSummary() throws Exception {
  ...
  void shouldValidateHighValueSales() throws Exception {
    logger.info("Running high-value sales validation test");

    // Insert additional related data
    try (final Connection connection = dataSource.getConnection();
        final var statement = connection.createStatement()) {
      statement.executeUpdate(
          """
   ...
   * and verifying the final state contains only January data. This demonstrates date-based
   * filtering where February data is intentionally excluded from the expected results.
   *
   * @throws Exception if database operation fails
   */
  @Test
  ...
  void shouldValidateJanuarySales() throws Exception {
    logger.info("Running January sales validation test");

    // Add January data only
    try (final Connection connection = dataSource.getConnection();
        final var statement = connection.createStatement()) {
      statement.executeUpdate(
          """
