...
final class AnnotationResolver {
  ...
  List<ScenarioName> resolveScenarioNames(final DataSet annotation, final Method testMethod) {
    final var normalized =
        Stream.of(annotation.scenarioNames())
            ...
            .toList();
    return normalized.isEmpty() ? List.of(new ScenarioName(testMethod.getName())) : normalized;
  }
  ...
  Optional<String> extractResourceLocation(final DataSet annotation) {
    final var location = annotation.resourceLocation();
  ...
  Optional<DataSourceName> resolveDataSourceName(final DataSet annotation) {
    final var dataSourceName = annotation.dataSourceName();
    return dataSourceName.isEmpty()
        ? Optional.empty()
        : Optional.of(new DataSourceName(dataSourceName));
  }
