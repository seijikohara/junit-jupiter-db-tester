...
/**
 * Resolves directory locations for CSV dataset files.
 *
 ...
 *   <li>With suffix: {@code classpath:com/example/service/UserServiceTest/expected/}
 * </ul>
 *
 * <h2>Validation</h2>
 *
 ...
 *   <li>The directory contains at least one CSV file
 * </ul>
 *
...
record DirectoryResolver(Class<?> testClass) {
   ...
   *     resolution
   * @param suffix the directory suffix to append, or {@code null} for no suffix (e.g., {@code null}
   *     or {@code ""} for preparation, {@code "expected/"} for expectation)
   * @return the resolved directory path containing CSV files
  ...
  /**
   * Validates that a directory contains at least one CSV file.
   *
   * @param directory the directory path to validate
   * @throws IllegalStateException if the directory contains no CSV files
   */
  void validateDirectoryContainsCsvFiles(final Path directory) {
    try (final var files = Files.list(directory)) {
      final var hasCsvFiles =
          files.anyMatch(path -> path.getFileName().toString().endsWith(".csv"));

      if (!hasCsvFiles) {
        final var message =
            String.format(
                """
                Dataset directory exists but contains no CSV files: '%s'

                Hint: Add at least one CSV file (e.g., TABLE_NAME.csv) to this directory. Each CSV file represents one database table.""",
            ...
            () -> {
              ...
              final var message =
                  String.format(
                      ...
                      Hint: Create the directory and add CSV files, or use @DataSet(resourceLocation = "...") to specify a custom location.""",
    ...
    if (!Files.exists(path)) {
      final var message =
          String.format(
              ...
              Hint: Create the directory and add CSV files, or verify the path is correct.""",
              location);
