...
class DataSetLoaderTest {
    ...
    class LoadMethod {
        ...
        @Test
        @Tag("normal")
        @DisplayName("Returns dataset when file exists")
        void returnsDataSet_whenFileExists() {
            // Given
        ...
        void throwsException_whenFileNotFound() {
            // Given
...
**Test Method Names**:
- Pattern: `<actionVerb><Object>_<condition>`
- Use descriptive names in camelCase
- No `test` prefix (use `@DisplayName` for readability)
- Examples:
  - `returnsDataSet_whenFileExists()`
  - `throwsException_whenNullArgument()`
  - `createsConnection_withValidDataSource()`
...
- English only
- Descriptive sentence format (not "should xxx when yyy")
- Example: `@DisplayName("Returns empty list when no data exists")`
...
@DisplayName("Loads dataset with valid paths")
void loadsDataSet_withValidPaths(final Path path) {
    final var result = loader.load(path);
    assertNotNull(result);
}
...
```java
@Test
void createsFile_inTempDirectory(@TempDir final Path tempDir) {
    // Given
...
@Test
@Tag("normal")
@DisplayName("Initializes with default configuration")
...
class DataSetLoaderTest {
    ...
    class LoadMethod {
        ...
        @Tag("normal")
        @DisplayName("Returns dataset when file exists")
        void returnsDataSet_whenFileExists() throws IOException {
            // Test implementation
        }
    }
}
...
@Test
@Tag("error")
@DisplayName("Throws exception with correct message")
void throwsExceptionWithMessage_whenInvalidInput() {
    final var exception = assertThrows(
        IllegalArgumentException.class,
        () -> loader.load(null)
    );
...
@Test
@Tag("normal")
@DisplayName("Returns present optional when found")
...
class ServiceTest {
    ...
    @Test
    @Tag("normal")
    @DisplayName("Calls validator and repository in order")
    void callsComponentsInOrder() {
        // Given
